...
# Ensure PyTorch MPS fallback is enabled on macOS if needed
...
def select_device():
    """Select appropriate device: MPS (Apple), CUDA (GPU), or CPU."""
...
def get_agent(env, config, device):
    """Factory method to create the appropriate agent from config."""
    if config["algo"] == "dqn":
...
def evaluate_agent(env, agent, num_episodes: int):
    """Run evaluation episodes with the trained policy and compute mean/std return."""
    total_rewards = []
...
def generate_final_summary(manager: ExperimentManager, config: Dict, args: argparse.Namespace, total_time: float, robustness_results: Dict) -> Dict:
    ...
    if not eval_df.empty:
        ...
        # Frames needed to reach a threshold performance
        THRESHOLD = 15.0
    ...
    # Robustness statistics (performance under different sticky-action settings)
...
def main(args):
    # --- 1. Load config and prepare experiment manager ---
    ...
    # Sync wall-clock time with existing log if resuming
    ...
    for frame_idx in range(start_frame, args.frames + 1):
        ...
        # Step environment depending on algorithm type
        ...
        # Update policy depending on algorithm
    ...
    # --- 7. After training: robustness checks & summary ---
...
if __name__ == "__main__":
    # CLI parser for launching training runs
